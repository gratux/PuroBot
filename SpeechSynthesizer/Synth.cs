...
using static Bisqwit.SpeechSynthesizer.SynthData;
using static Bisqwit.SpeechSynthesizer.Helpers;
...
namespace Bisqwit.SpeechSynthesizer
{
    ...
    public class Synth
    {
        ...
        private IEnumerable<byte> VocalizePhonemes(IEnumerable<ProsodyElement> phonemes)
        {
            ...
            double tokenPitch = TokenPitch;
            double breathBaseLevel = BreathBaseLevel;
            double voiceBaseLevel = VoiceBaseLevel;
            ...
            foreach (ProsodyElement e in phonemes)
            {
                ...
                double buzz = record.Voice * voiceBaseLevel;
                double pitch = e.RelativePitch * tokenPitch;
            ...
            int nSamples)
        {
            ...
            while (true)
            {
                ...
                for (int n = 0; n < nSamples; n++)
                {
                    ...
                    double gain = GetWithHysteresis(_pGain, hysteresis);
                    ...
                    _offset++;
                    double r = _bp[_offset %= MaxOrder] = sum;

                    double sample = r * Math.Sqrt(GetWithHysteresis(_pGain, hysteresis)) * short.MaxValue;
        ...
        private double GetWithHysteresis(double value, int speed)
        {
            ...
            if (!_last.TryGetValue(value, out double oldValue))
            {
                _last.Add(value, newValue);
                return newValue;
            }

            double newFac = Math.Pow(2, speed);
        ...
        private static IEnumerable<ProsodyElement> Phonemize(string text)
        {
            ...
            for (int pos = 0; pos < input.Count; pos++)
            {
                ...
                bool surroundedByVowel = nextElement != input[pos] && IsVowel(nextElement) || pos > 0 && IsVowel(input[pos - 1]);

                if (Maps.ContainsKey(input[pos]))
                    foreach (var element in Maps[input[pos]])
                    {
                        yield return new ProsodyElement(element.Character,
                            element.Length + element.RepeatedLength * repLenFunc() + (surroundedByVowel ? element.SurroundedLength : 0),
        ...
        private static void EnglishConvert(Utf32String text)
        {
            ...
            {
                int ruleIdx = IsAlphabet(row[1][0]) ? 1 + (row[1][0] - 'a') : 0;
        ...
        private static int ApplyConversionRules(Utf32String text, int position, List<Utf32String[]>[] rules, Utf32String result)
        {
            bool ruleMatched = false;
            int ruleIdx = IsAlphabet(text[position]) ? 1 + (text[position] - 'a') : 0;
            foreach (var rule in rules[ruleIdx])
            {
                Utf32String lPattern = rule[0], mPattern = rule[1], rPattern = rule[2];

                bool lMatch = MatchesRulePattern(rPattern, text.GetEnumerator());
                bool mMatch = text.Matches(mPattern, position, mPattern.Count);
        ...
        private static void ChangeAccentAsync(Utf32String result, Dictionary<Utf32String, Utf32String> accentRules)
        {
        ...
        private static bool ApplyCanonizationRulesAt(int position, Utf32String result, Dictionary<Utf32String, Utf32String> rules)
        {
