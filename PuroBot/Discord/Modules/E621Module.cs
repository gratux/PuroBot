...
namespace PuroBot.Discord.Modules
{
    public class E621Module : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task E621DmOrGroupCommand([Summary("the number of posts to request (default: 5)")] int count = 5,
            [Remainder] [Summary("the tags to be searched (default: none)")] string tags = "")
        {
        ...
        [RequireNsfw]
        public async Task E621GuildCommand([Summary("the number of posts to request (default: 5)")] int count = 5,
            [Remainder] [Summary("the tags to be searched (default: none)")] string tags = "")
        {
            await E621Command(count, tags, false);
        }
        ...
        public async Task E926Command([Summary("the number of posts to request (default: 5)")] int count = 5,
            [Remainder] [Summary("the tags to be searched (default: none)")] string tags = "")
        {
        ...
        public async Task SelfieCommand([Summary("minimum e926 score (default: 10)")] int minScore = 10)
        {
            var posts = await GetPostUrls(true, $"puro_(changed) solo score:>{minScore}", 1).ToListAsync();

            string? url = posts.FirstOrDefault();
            if (url is null)
            {
