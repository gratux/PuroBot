...
namespace PuroBot.Discord.Modules
{
    public class HelpModule : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task HelpCommand()
        {
            ...
            string prefix = Context.IsPrivate
        ...
        private static Task<string> CommandHelp(CommandInfo command)
        {
            ...
            string summary = command.Summary ?? "No summary";
            string[] aliases = command.Aliases.Skip(1).ToArray();
            var parameterInfos = command.Parameters;
            string[] preconditions = command.Preconditions.Concat(command.Module.Preconditions)
                .Where(p => p is not RequireContextAttribute)
                .Select(DiscordExtensions.DecodePrecondition)
                .Distinct()
                .ToArray();
